...
Find and retrieve relevant documentation from the Polars corpus.
...
- **Synonyms**: Related terms (e.g., "filter" → also try "select", "query", "where")
- **Domain terms**: Technical concepts that might appear in docs (e.g., "lazy" → also try "LazyFrame", "collect")
...
**IMPORTANT:** If you searched for "streaming" and only found "I/O" without "streaming" in the description, that's a **related hit, not a direct hit**. Stop and ask the user before exploring further.
...
> I searched the index for "streaming", "sink", and "parquet" but found no direct matches.
>
> **Options:**
> 1. **Rephrase**: Did you mean "lazy evaluation" or "write operations"?
> 2. **Related sections**: The "I/O" section covers CSV, JSON, and database connectors
...
Examples:
- `polars:reference/hooks.md` - Git source
...
3. The project will now know to use this corpus for Polars questions

### When to Suggest Injection

Suggest adding project awareness when:
- User is working in a project that heavily uses Polars
- User repeatedly asks Polars questions without invoking the corpus
- User says "I keep forgetting to use the docs"

Example suggestion:
> "I notice this project uses Polars extensively. Would you like me to add corpus awareness to this project's CLAUDE.md? That way I'll automatically know to check the Polars docs when working here."
