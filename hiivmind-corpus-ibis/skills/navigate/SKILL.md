...
**IMPORTANT:** If you searched for "window functions" and only found "Expressions" without "window" in the description, that's a **related hit, not a direct hit**. Stop and ask the user before exploring further.
...
> I searched the index for "window functions", "ranking", and "partition" but found no direct matches.
>
> **Options:**
> 1. **Rephrase**: Did you mean "aggregations" or "analytics"?
> 2. **Related sections**: The "Expressions" section covers common operations
...
**Index entry found:** `ibis:backends/duckdb.qmd`

**Step 1 - Parse the path:**
- `source_id` = `ibis` (everything before the colon)
- `relative_path` = `backends/duckdb.qmd` (everything after the colon)
...
- Local clone: `.source/ibis/docs/backends/duckdb.qmd`
- GitHub URL: `https://raw.githubusercontent.com/ibis-project/ibis/main/docs/backends/duckdb.qmd`
